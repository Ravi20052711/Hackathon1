"""
=============================================================
LLM ANALYZER - Multi-AI Support
=============================================================
Priority order:
1. Groq (FREE - no credit card) - https://console.groq.com
2. Claude (Anthropic) - paid but cheap
3. Gemini (Google) - free but quota issues in India
4. Template fallback - always works

Add to backend/.env:
  GROQ_API_KEY=gsk_your-key-here   ← FREE, get at groq.com
=============================================================
"""
...
class LLMAnalyzer:
    ...
    async def _call_groq(self, prompt: str, label: str) -> str:
        ...
        Uses Llama 3.3 70B model — very capable.
        ...
        async with aiohttp.ClientSession() as session:
            async with session.post(
                ...
                json={
                    ...
                    "messages": [
                        {"role": "system", "content": "You are a senior threat intelligence analyst. Be concise and technical."},
    ...
    def _build_prompt(self, raw_log, iocs, matches, techniques) -> str:
        context = []
        if matches:
            context.append(f"Known threats matched: {json.dumps([{'value': m['value'], 'type': m['ioc_type'], 'risk': m['risk_score'], 'source': m['source']} for m in matches])}")
        if iocs.get('ips'): context.append(f"IPs: {', '.join(iocs['ips'][:5])}")
        if iocs.get('domains'): context.append(f"Domains: {', '.join(iocs['domains'][:5])}")
        if iocs.get('hashes'): context.append(f"Hashes: {', '.join(iocs['hashes'][:3])}")
        if techniques: context.append(f"MITRE: {', '.join(techniques)}")
        return f"""Analyze this security alert:

RAW LOG: {raw_log[:500]}

CONTEXT: {chr(10).join(context) if context else 'None'}

Provide:
1. THREAT ASSESSMENT (2-3 sentences)
2. SEVERITY JUSTIFICATION
3. ATTACK STAGE (kill chain phase)
4. IMMEDIATE ACTIONS (2-3 steps)"""
    ...
    def _template_summary(self, iocs, matches, techniques) -> str:
        ...
        if not lines: lines.append("ℹ️ No specific indicators matched.")
        lines.append("\n💡 Get free AI: Sign up at console.groq.com → add GROQ_API_KEY to .env")
        return '\n'.join(lines)
